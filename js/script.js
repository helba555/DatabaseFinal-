    ...
    if (comment != "") {
      var $newComment = $(`
            ...
            <div class="ms-3" id="${user.replace(/\s/g, "_")}">
                <div class="fw-bold"><span class="person-name"  onclick="handleClick(event.target.parentElement.parentElement)">${user}</span>
                <button type="button" class="btn btn-danger del-comment">Delete comment</button></div>
                <div class="comment-time" id="${Date.now()}">${
        humanizeDuration(0, { round: true }).split(",")[0]
      }</div>
      ...
      if (!comment.split("\n")[0].startsWith("@")) {
        ...
        console.log(name);
        document.getElementById(name).querySelector(".child").innerHTML +=
          child(Date.now(), user, message, userAvatar);
        $("#post-comment").val("");
      }
    }
  });
...
// Delete comment
function delFunc(className) {
  const delElement = document.querySelectorAll(".btn-danger");
  console.log(delElement);

  if (delElement) {
    delElement.forEach((elem) => {
      elem.addEventListener("click", () => {
        console.log("sdfm ");
        elem.closest(className).remove();
      });
    });
  }
}
