...
function handleClick(ev) {
  document.getElementById("post-comment").value = `@${
    ev.querySelector(".person-name").innerText
  }\n`;
}
...
function addPost(title, post, imageFile) {
  return `
    ...
    <img src="${URL.createObjectURL(
      imageFile
    )}" style="max-width:100%;height:auto" class="card-img-top" alt="...">
    <div class="card-body">
...
try {
  document.querySelectorAll(".comment-time").forEach((element) => {
    element.innerHTML =
      humanizeDuration(Date.now() - Number(element.id), { round: true }).split(
        ","
      )[0] + " ago";
  });
} catch (error) {}
